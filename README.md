...
커뮤니티 비서의 예약 실행은 별도 GitHub Actions workflow로 관리하며, 예약 시각보다 늦게 시작될 수 있어 정각 발행을 보장하지 않습니다. `COMMUNITY_AGENT_SCHEDULE_ENABLED`가 `true`일 때만 수집·작성·발행을 실행하고, 그 외에는 보존 기한 정리만 실행합니다. GitHub Actions가 비활성화되었거나 앱에 연결할 수 없으면 자동 정리도 실행되지 않으므로, 운영자가 `community:run -- --cleanup-only`로 수동 정리를 해야 합니다.

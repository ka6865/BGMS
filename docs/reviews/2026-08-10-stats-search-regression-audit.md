...
| STATS-009 | fixed | `/stats/kakao/Fixture%20Player` → “전적 비교” | `/stats/battle?nick1=Fixture%20Player&platform1=kakao` push | `StatsPageShell` profile action이 Task 2 `buildStatsCompareUrl` 결과를 그대로 push | 0건 | `StatsPageShell.tsx`, `statsPageModel.ts`, `PlayerProfileHeader.tsx` | 기존 inline 비교 URL이 platform을 누락했으나 Task 6에서 profile action을 platform-aware builder로 교체 | `player-profile-header` Kakao compare·spaced nickname URL 회귀 | Shell 최종 조립에서도 builder wiring을 단일 action으로 유지 |
...
| STATS-011 | not_reproduced | SSR `/stats`, localStorage에 `['StoredPlayer']` 저장 후 reload | hydration warning 0건 | reload 후 hydration 관련 page error 0건; input boundary 유지 | landing player `0/0/0/0`; subsequent dynamic route player `2/2/0/2` (started/completed/aborted/successful), summary `2/2/0/2`; analytics 9 aborted | `useStatsSearchHistory.ts`, `StatsSearchBar.tsx` | 이번 브라우저 실행에서 hydration 결함 미재현 | `stats-browser-smoke` STATS-011-012 row, fresh context/reload | hydration 결함 미재현; production 변경 없음 |
| STATS-012 | not_reproduced | `/stats` 검색 후 뒤로가기·문서 metadata 확인 | URL, 화면, metadata 동기화 | dynamic URL 진입 후 back으로 `/stats`, forward로 `/stats/steam/FixturePlayer` 복귀; `document.title`, canonical, `og:url` 동기화 | dynamic player `2/2/0/2`, summary `2/2/0/2`; landing player 0건 | `StatSearch.tsx` `navigateToPlayer`, dynamic route metadata | 해당 브라우저 run에서 history/metadata 불일치 미재현 | `stats-browser-smoke` STATS-011-012 row; exact URL/DOM metadata assertions | suspected 유지에서 not_reproduced로 판정 |
| STATS-013 | not_reproduced | direct `/stats/xbox/FixturePlayer` | `/stats` redirect 및 PUBG player 요청 0건 | `/stats`로 redirect; player 요청 0건 | player `0/0/0/0`; analytics 2 aborted | `app/stats/[platform]/[nickname]/page.tsx`, route validation | invalid platform external fetch/500 경로 미재현 | `stats-browser-smoke` STATS-013 row | suspected 유지에서 not_reproduced로 판정 |
| STATS-014 | fixed | ready 결과에서 시즌 변경, player API 500 | 이전 결과 유지+오류 안내 | 500 전후 profile title/nickname/platform, match ID, compact row text가 exact equal이고 error UI가 표시됨 | base player `1/1/0/1`, summary `1/1/0/1`; season player `1/1/0/0` status 500; analytics 2 aborted | `StatSearch.tsx` refresh/season controller 경계 | refresh/season failure가 이전 result를 제거하지 않도록 controller가 보존 | `stats-browser-smoke` STATS-014 row; exact profile/match-row snapshot·error·ledger assertions | browser evidence로 fixed 판정 |
...
| STATS-016 | not_reproduced | B ready → 실제 검색 button `router.push(A)` → `page.goBack()` B, A는 600ms delay | 기존 App Router remount 경계에서 B만 유지 | same document/navigation entry 1개이지만 dynamic route별 Shell은 remount; A는 in-document `fetch-signal` abort/successful 0, B는 `afterRecordId` 이후 신규 200/successful 1건으로 heading/URL 복귀 | A player `1/0/1/0`; B player total `2/2/0/2`; B summary `2/2/0/2`; analytics 2 aborted | 기존 `app/stats/[platform]/[nickname]/page.tsx` remount 경계, controller unmount AbortController | 최신 B UI를 늦은 A가 덮는 결함은 실제 client route에서 미재현; persistent Shell 구조 변경은 범위 제외 | `stats-browser-smoke` route-remount 390/1440 + STATS-016 audit; exact terminal source/afterRecordId/DOM assertions | 생산 수정 없이 `not_reproduced`; same-instance race로 `fixed`를 주장하지 않음 |
...
## Task 10 실제 browser evidence 보강

2026-08-11 same-Shell 검토에서 기존 App Router가 dynamic route별 Shell을 remount함을 확인했고, 테스트를 위한 persistent layout 구조는 범위에서 제거했다. 제거 후 영향 범위 browser run은 route-remount 390/1440 + audit `3 passed / 40 skipped / exit 0`이었고, 현재 코드의 마지막 fresh full은 `RUN_STATS_BROWSER_SMOKE=true npx vitest run tests/stats-browser-smoke.test.ts --testTimeout=120000 --silent=false --reporter=dot`로 `43 passed / 0 failed / exit 0 / 64.08s`였다. 기능 viewport는 `390×844`, `1440×900`, layout viewport는 `375×667`, `390×844`, `430×932`, `768×1024`, `1440×900`, `1600×900`, resize는 `767↔768`, `1023↔1024`, screenshot-only는 `375×812`, `1280×720`, `1920×1080`이었다.

실제 audit ledger snapshot은 다음과 같다. 표기의 순서는 `started/completed/aborted/successful`이며 HTTP 500은 completed이지만 successful이 아니다.

| row | 실제 ledger | 실제 UI/metadata | 판정 |
| --- | --- | --- | --- |
| STATS-011 | landing player `0/0/0/0`; subsequent dynamic player `2/2/0/2`; summary `2/2/0/2`; analytics 9 aborted; ad 0; other 0 | localStorage reload 후 hydration 관련 page error 0, input boundary 유지 | `not_reproduced` |
| STATS-012 | 위 dynamic route snapshot의 player `2/2/0/2`, summary `2/2/0/2` | `/stats/steam/FixturePlayer` → back `/stats` → forward dynamic URL; title, canonical, og:url 일치 | `not_reproduced` |
| STATS-013 | player `0/0/0/0`; analytics 2 aborted; ad 0; other 0 | `/stats/xbox/FixturePlayer`가 `/stats`로 redirect | `not_reproduced` |
| STATS-014 | base player `1/1/0/1`; summary `1/1/0/1`; season player `1/1/0/0`, status 500; analytics 2 aborted | 500 전후 profile title/nickname/platform, match ID, compact row text exact equal + 오류 표시 | `fixed` |
| STATS-016 | A player `1/0/1/0` (`fetch-signal`); B player total `2/2/0/2`; B summary `2/2/0/2`; analytics 2 aborted; ad 0; other 0 | same document/nav entry 1, Shell remount; 신규 B `afterRecordId` 200 후 heading/URL 유지 | `not_reproduced` |

브라우저 smoke에서 실제로 assertion한 UI 경계는 team/weapon/tier/AI section labels와 controls, loading status, partial error alert, ready heading/ad/match state, squad groups/groupKey, unauthenticated AI zero-request다. 2D/3D/map의 telemetry-backed navigation과 data rendering, authenticated AI result clamp/finalVerdict는 실행하지 않았고 `미검증`으로 남긴다.

Control-size evidence는 전체 named controls를 측정했다. 390px mobile에서 실제 nested legacy list는 `match-filters` (`41.78×44`)와 `bottom-nav` (`74.81×38`)이며 desktop blocking list는 빈 목록이었다. collapsed/not-applicable controls는 missing을 pass로 세지 않고 분류만 했다. 새 Shell/States primary control violation은 관찰되지 않았다.

Layout assertion의 실제 결과는 각 layout/screenshot/resize row에서 `globalOverflow=0`, `internalOverflow=[]`, shell cap `≤1200`, mobile top reservation `100px`, tablet+ `90px`, fluid reservation `≥130px`, safe padding/BottomNav overlap pass였다. 광고 외부 요청은 `0`; analytics는 별도 집계했다. `adfit`/`mounted`/`DAN-fixture-stats-feed-728`는 local placeholder registry evidence일 뿐 live fill evidence가 아니다.

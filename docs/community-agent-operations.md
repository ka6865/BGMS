...
한 실행의 Gemini 호출은 분류·작성·검증 합계 최대 3회이며 실패 호출도 소비량에 포함된다. 모델 fallback이나 자동 유료 전환은 없다. 현재 개발 smoke 5회에서 기록된 사용량은 prompt 12,713, completion 1,634, 합계 14,347 tokens였다. 별도의 최소 연결 확인 1회는 사용량을 기록하지 않았고, 이 수치를 제공자 청구량으로 보지 않는다. 무료 tier 안에서 실행했으므로 확인된 금전 지출은 0원이지만 무료 quota는 소비했다. 실제 활성화 전 현재 가격과 quota를 다시 확인한다.

-- PC patch windows. No historical sample is deleted, copied or rewritten.
CREATE TABLE public.weapon_meta_patches (
  version text PRIMARY KEY CHECK (length(version) BETWEEN 1 AND 32),
  maintenance_started_at timestamptz NOT NULL UNIQUE,
  starts_at timestamptz NOT NULL UNIQUE,
  source_url text,
  timing_status text NOT NULL DEFAULT 'scheduled' CHECK (timing_status IN ('scheduled','confirmed','historical')),
  CHECK (starts_at >= maintenance_started_at)
);
ALTER TABLE public.weapon_meta_patches ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.weapon_meta_patches FROM PUBLIC,anon,authenticated;
GRANT ALL ON public.weapon_meta_patches TO service_role;
INSERT INTO public.weapon_meta_patches(version,maintenance_started_at,starts_at,source_url,timing_status) VALUES
 ('42.3','2026-08-12T03:00:00Z','2026-08-12T03:00:00Z',NULL,'historical'),
 ('43.1','2026-09-10T00:00:00Z','2026-09-10T08:30:00Z','https://www.pubg.com/ko/news/11057','scheduled');
CREATE INDEX IF NOT EXISTS idx_weapon_meta_samples_played_type
 ON public.weapon_meta_match_samples(played_at,match_type);

-- Collection uses the match date, even if an older app still sends a static patch label.
-- Reports independently use dates, so adjusting a scheduled boundary needs no row rewrite.
CREATE FUNCTION public.assign_weapon_meta_patch() RETURNS trigger
LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $$
DECLARE patch text;
BEGIN
 SELECT version INTO patch FROM public.weapon_meta_patches
 WHERE starts_at<=NEW.played_at ORDER BY starts_at DESC LIMIT 1;
 IF patch IS NOT NULL THEN NEW.patch_version:=patch; END IF;
 RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.assign_weapon_meta_patch() FROM PUBLIC,anon,authenticated;
CREATE TRIGGER assign_weapon_meta_patch BEFORE INSERT OR UPDATE OF played_at,patch_version
 ON public.weapon_meta_match_samples FOR EACH ROW EXECUTE FUNCTION public.assign_weapon_meta_patch();

CREATE FUNCTION public.get_weapon_meta_patch_report(

-- Preserve pre/post separation even on the same Korean calendar day.
 ...
 WITH selected AS MATERIALIZED (
  ...
  WHERE s.played_at>=target.maintenance_started_at-interval '14 days'
   ...
   AND (p_match_type='all' OR s.match_type=p_match_type)
    ...
    SELECT (played_at AT TIME ZONE 'Asia/Seoul')::date AS date,period,
      count(DISTINCT (match_id,platform,player_id)) AS player_match_count
    FROM selected GROUP BY 1,2
  ), daily AS (
    SELECT to_char(d.date,'YYYY-MM-DD') AS date,d.period,r.scope,r.scope_name AS weapon_name,
    ...
    FROM scope_rows r JOIN daily_denominators d ON (r.played_at AT TIME ZONE 'Asia/Seoul')::date=d.date AND r.period=d.period
    GROUP BY d.date,d.period,r.scope,r.scope_name,d.player_match_count
  ), scopes AS (
  ...
  SELECT jsonb_build_object(
    ...
    'dailyWeaponTrend',coalesce((SELECT jsonb_agg(to_jsonb(d) ORDER BY date,period DESC,scope,weapon_name) FROM daily d),'[]'::jsonb),

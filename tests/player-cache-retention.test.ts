...
describe("pubg_player_cache 보존 정책", () => {
  ...
  it("한 실행의 총 삭제 규모를 3만 행 수준으로 유지한다", async () => {
    ...
    const many = Array.from({ length: 200 }, () => compactionResult(500, 250_000));
    ...
    expect(totalDeletable).toBeGreaterThanOrEqual(10_000);
    expect(totalDeletable).toBeLessThanOrEqual(30_000);
  });

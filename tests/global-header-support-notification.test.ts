...
vi.mock("@/lib/supabase", () => ({
  supabase: {
    from: (table: string) => table === "profiles"
      ? { select: () => ({ eq: () => ({ single: async () => ({ data: { id: "user-1", nickname: "사용자", role: profileRole }, error: null }) }) }) }
...
beforeEach(() => { vi.clearAllMocks(); authState = { user: { id: "user-1" }, loading: false }; profileRole = "user"; });
afterEach(() => cleanup());
...
describe("GlobalHeader support notifications", () => {
  ...
  it("shows the admin support inbox shortcut only for admins", async () => {
    profileRole = "admin";
    render(React.createElement(GlobalHeader));

    await waitFor(() => expect(screen.getByRole("link", { name: /고객센터 관리/ })).toHaveAttribute("href", "/admin/support"));
  });
});

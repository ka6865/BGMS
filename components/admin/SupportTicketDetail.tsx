...
export default function SupportTicketDetail({ ticketId, onChanged }: { ticketId: string; onChanged?: () => void }) {
  ...
  async function postMessage() {
    ...
    setBusy(true); setError("");
    try {
      const response = await fetch(`/api/admin/support/tickets/${ticketId}/messages`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ body: reply }) });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "답변을 저장하지 못했습니다.");
      setReply(""); await load(); onChanged?.(); toast.success("답변을 보냈습니다.");
  ...
  return <section aria-label="문의 상세" className="rounded-2xl border border-white/10 bg-[#151515] p-5 text-white sm:p-6">
    ...
    {isPrivacy && <div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-4 text-sm"><p className="font-bold text-amber-100">대상 계정</p><p className="mt-1 text-white/75">{ticket.target_platform ?? "-"} · {ticket.target_nickname ?? "-"}</p><p className="mt-1 text-xs text-white/45">{ticket.target_account_id ?? "계정 ID 없음"}</p><div className="mt-3 flex flex-wrap gap-2">{(ticket.attachments ?? []).map((attachment) => attachment.signedUrl ? <a key={attachment.id} href={attachment.signedUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-white/15 px-3 py-2 text-xs text-amber-100">{attachment.original_name} 열기</a> : <span key={attachment.id} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-white/45">{attachment.original_name}</span>)}</div></div>}
    <div className="mt-5 space-y-3"><p className="whitespace-pre-wrap rounded-xl bg-black/20 p-4 text-sm leading-6 text-white/75">{ticket.body}</p>{(ticket.messages ?? []).map((message) => <article key={message.id} className={`rounded-xl p-4 ${message.sender_type === "admin" ? "bg-amber-300/[0.08]" : "bg-white/[0.04]"}`}><p className="text-xs font-bold text-white/45">{message.sender_type === "admin" ? "관리자" : "요청자"}</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-white/80">{message.body}</p></article>)}</div>
    {error && <p role="alert" className="mt-4 rounded-lg border border-red-400/25 bg-red-400/10 p-3 text-sm text-red-200">{error}</p>}
    <div className="mt-5 flex flex-col gap-3"><label className="text-sm font-bold">관리자 답변<textarea aria-label="관리자 답변" value={reply} onChange={(event) => setReply(event.target.value)} rows={4} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 p-3 font-normal" /></label><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void postMessage()} disabled={busy || !reply.trim()} className="min-h-11 rounded-xl bg-amber-400 px-4 text-sm font-black text-black disabled:opacity-40">답변 보내기</button>{isPrivacy && <button type="button" onClick={() => void applyPrivacyAction()} disabled={busy || !isVerified} className="min-h-11 rounded-xl border border-amber-300/40 px-4 text-sm font-black text-amber-100 disabled:cursor-not-allowed disabled:opacity-40">비공개 목록에 등록</button>}</div></div>
  </section>;
}

...
export default function TicketThread({ ticketId, initialTicket }: { ticketId: string; initialTicket?: Ticket }) {
  ...
  const [messageIdempotencyKey, setMessageIdempotencyKey] = useState("");
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    setError("");
    const requestKey = messageIdempotencyKey || createMessageIdempotencyKey();
    setMessageIdempotencyKey(requestKey);
    setSending(true);
    try {
      ...
      setBody("");
      setMessageIdempotencyKey("");
      setRefresh((value) => value + 1);
    } catch {
      setError("메시지를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요.");
    } finally {
      setSending(false);
    }
  }

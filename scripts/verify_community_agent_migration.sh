...
"${PSQL[@]}" -c "begin; select public.publish_community_post('${RUN_ID}'::uuid); select pg_sleep(1); commit;" >/tmp/community-agent-publish-a-$$.out &
PUBLISH_A=$!
sleep 0.2
"${PSQL[@]}" -c "select public.publish_community_post('${RUN_ID}'::uuid);" >/tmp/community-agent-publish-b-$$.out &
PUBLISH_B=$!
wait "$PUBLISH_A"
wait "$PUBLISH_B"
...
"${PSQL[@]}" -c "begin; select public.publish_community_post('${STOP_RUN_ID}'::uuid); select pg_sleep(1); commit;" >/tmp/community-agent-stop-publish-a-$$.out &
STOP_PUBLISH_A=$!
sleep 0.2
"${PSQL[@]}" -c "begin; update public.community_agent_policy set enabled = false, publishing_enabled = false where singleton; commit;" >/tmp/community-agent-stop-publish-b-$$.out &
STOP_PUBLISH_B=$!
wait "$STOP_PUBLISH_A"
wait "$STOP_PUBLISH_B"
